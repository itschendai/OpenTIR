#!/usr/bin/env python3
"""LightGreen injectable teardown recipe.
...
Real hardware execution intentionally prompts before CUT_HEIGHT actions unless
``--yes`` is passed.
"""
...
REMOVAL_CYCLES = (
    ...
    },
    {
        ...
        "cut_x_param": "YELLOW_CUT_X_MM",
        "cut_z_param": "YELLOW_CUT_Z_MM",
        "cut_deg_param": "YELLOW_CUT_DEG",
...
for cycle in REMOVAL_CYCLES:
    ...
    if "cut" in steps:
        cycle_plan_steps[int(steps["cut"])] = f"CUT_HEIGHT for {name} removal"
    PLAN_STEPS.update(cycle_plan_steps)
...
PLAN_STEPS.update(
    {
        38: "move to Middle for shell and glass removal",
        39: "move to above Vise for shell and glass removal",
        40: "move to Vise grasp for shell and glass removal",
        41: "close gripper for shell and glass removal",
        42: "open vise before lifting shell",
        43: "lift shell up in positive world Z",
        44: "move to Middle after shell removal",
        45: "move to Glass",
        46: "dump with MoveC arc around tool-frame virtual pivot and return",
        47: "move to Plastic",
        48: "open gripper",
        49: "move to Middle",
    }
...
ARDUINO_REQUIRED_STEPS = {10, 13, 42}
for cycle in REMOVAL_CYCLES:
    cut_step = dict(cycle["steps"]).get("cut")
    if cut_step is not None:
        ARDUINO_REQUIRED_STEPS.add(int(cut_step))

GRIPPER_STEP_NUMBERS = {1, 5, 11, 12, 15, 20, 41, 48}
for cycle in REMOVAL_CYCLES:
    steps = dict(cycle["steps"])
    GRIPPER_STEP_NUMBERS.update({int(steps["close"]), int(steps["open"])})
...
class SingleInstanceLock:
    ...
    def acquire(self) -> None:
        ...
        try:
            ...
            raise RecipeError(
                "another LightGreen recipe process is already running"
                f"{detail}"
            )
        handle.seek(0)
        handle.truncate()
...
def make_logger():
    try:
        ...
        return spdlog.ConsoleLogger("LightGreenRecipe")
    except Exception:
        return PrintLogger()
...
def step_requires_arduino(step_number: int) -> bool:
    return step_number in ARDUINO_REQUIRED_STEPS


def step_uses_gripper(step_number: int) -> bool:
    return step_number in GRIPPER_STEP_NUMBERS
    ...
    *,
    ...
    if removal_step is not None:
        ...
        if action == "transit":
            ...
            run_cut_height(
                ctx,
                cut_label=f"CUT_HEIGHT({name.lower()})",
                z_mm=float(ctx.params[str(cycle["cut_z_param"])]),
                x_mm=float(ctx.params[str(cycle["cut_x_param"])]),
                deg=float(ctx.params[str(cycle["cut_deg_param"])]),
                yes=yes,
                skip_cut=skip_cut,
            )
        elif action == "grip":
...
def confirm_cut(ctx: RecipeContext, yes: bool, *, cut_label: str = "CUT_HEIGHT") -> None:
    if ctx.dry_run or yes:
        return
    print(
        f"\n{cut_label} will fire the ultrasonic blade. "
        "Type CUT to continue, anything else to abort: ",
        end="",
        flush=True,
    )
    if input().strip() != "CUT":
        raise RecipeError(f"operator aborted before {cut_label}")
...
def run_cut_height(
    ...
    *,
    ...
    if status.get("status_query_unavailable"):
        ctx.logger.warn(
            f"{cut_label} pre-status unavailable because GET_STATUS is unsupported; "
            "skipping vise-state precheck and assuming the machine is ready."
        )
    else:
        ctx.logger.info(f"{cut_label} pre-status: {status}")
        if status.get("vise_state") != "CLOSED":
            message = f"cannot cut unless vise is CLOSED: {status}"
            if ctx.dry_run:
                ctx.logger.warn(f"{message}; dry-run will continue to show {cut_label}")
            else:
                raise RecipeError(message)
    ctx.logger.info(f"{cut_label} prep: ROTATE_ABS deg=0.0")
    arduino.rotate_abs(0.0)
    if skip_cut:
        ctx.logger.warn(f"Skipping {cut_label} because --skip-cut was supplied")
        return
    confirm_cut(ctx, yes=yes, cut_label=cut_label)
    ctx.logger.info(
        f"{cut_label} fire: z_mm={float(z_mm)} x_mm={float(x_mm)} deg={float(deg)}"
    )
    result = arduino.cut_height(z_mm=float(z_mm), x_mm=float(x_mm), deg=float(deg))
    ctx.logger.info(f"{cut_label} result: {result}")
...
def phase_remove_yellow_plastic(ctx: RecipeContext) -> None:
    ctx.logger.info("=== phase_cut_and_remove_yellow_plastic ===")
    run_plan_steps(ctx, (29, 30, 31, 32, 33, 34, 35, 36, 37))
...
def run_full_recipe(
    ...
    *,
    ...
    if uncut:
        ctx.logger.info(
            "=== uncut shortcut enabled: after step 12 jump directly to step 38 ==="
        )
        if frame_gated:
...
def run_robot_only_smoke(ctx: RecipeContext, *, uncut: bool = False) -> None:
    ...
    if uncut:
        ctx.logger.info(
            "Robot-only smoke uncut path: after step 12 jump directly to step 38"
        )
    for step_number in step_numbers:
    ...
    *,
    ...
    if uncut:
        ...
        logger.info(
            ...
            "cut_and_remove_yellow_plastic -> remove_shell_and_glass -> shutdown"
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="recipe.py",
        description="Run the LightGreen injectable robot + cutting-machine recipe.",
    )
    ...
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Skip CUT_HEIGHT confirmation prompts",
    )
    parser.add_argument(
        "--frame-gated",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-cut",
        action="store_true",
        help="Skip all machine CUT_HEIGHT actions",
    )
    parser.add_argument(
        "--uncut",
        action="store_true",
        help="After step 12, jump directly to step 38 and skip steps 13 through 37.",
    )
    parser.add_argument(
        "--slow",
        action="store_true",
    ...
    parser.add_argument("--yellow-cut-x-mm", type=float, default=None)
    parser.add_argument("--yellow-cut-z-mm", type=float, default=None)
    parser.add_argument("--yellow-cut-deg", type=float, default=None)
...
def resolve_params(args: argparse.Namespace) -> dict:
    ...
    overrides = {
        ...
        "YELLOW_CUT_X_MM": args.yellow_cut_x_mm,
        "YELLOW_CUT_Z_MM": args.yellow_cut_z_mm,
        "YELLOW_CUT_DEG": args.yellow_cut_deg,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.dry_run:
            run_lock = SingleInstanceLock(HERE / ".lightgreen.lock")
            run_lock.acquire()
