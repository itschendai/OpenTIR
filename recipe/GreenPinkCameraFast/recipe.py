...
PARAMS: dict[str, Any] = {
    ...
    # Frame 5 (steps 18-48: spring/plastic/shell/glass removal + dump) handles
    # already-processed scrap, so its MovePTP transits and MoveL lifts/descents
    ...
    "VISE_CLOSE_FLOATING_AXES": ["y", "z", "rx"],
...
PLAN_STEPS = {
    ...
    10: "close vise while floating in TCP Y-Z-RX",
...
class DryRunArduino:
    ...
    def close_vise(self, target_force_kg: float = 5.0) -> dict:
...
def offset_pose_tcp_axis(pose_entry: dict, axis: str, distance_m: float) -> dict:
    """Return a pose copy translated along one signed TCP axis."""
    tcp = list(pose_entry["tcp_pose_world"]["values"])
    if len(tcp) != 7:
        raise RecipeError(f"tcp_pose_world.values must have 7 numbers, got {len(tcp)}")
    ...
    if axis_text not in axis_to_col:
        raise RecipeError(f"unsupported TCP axis {axis!r}; expected x, y, z, -x, -y, or -z")
...
def cap_grip_world_pose(ctx: RecipeContext) -> dict:
    legacy_offset = ctx.params.get("CAP_GRIP_Z_OFFSET_M")
    world_offset_m = (
        float(legacy_offset)
        if legacy_offset is not None
        else float(ctx.params["CAP_GRIP_WORLD_Z_OFFSET_M"])
    )
    return vise_offset_pose(ctx, world_offset_m)


def cap_grip_tcp_pose(ctx: RecipeContext, base_pose: dict) -> dict:
    return offset_pose_tcp_axis(
        base_pose,
        "z",
        float(ctx.params["CAP_GRIP_TCP_Z_OFFSET_M"]),
    )
...
def step_requires_arduino(step_number: int) -> bool:
    return step_number in {10, 12, 13, 41}
...
FRAME5_FIRST_STEP = 18


def frame5_velocity_overrides(
    ctx: RecipeContext, step_number: int
) -> tuple[int | None, float | None]:
    """Frame-5 (steps 18-48) runs faster on scrap removal.
    ...
    """
    if bool(ctx.params["FRAME5_FAST_ENABLED"]) and step_number >= FRAME5_FIRST_STEP:
        return (
...
def execute_plan_step(
    ...
    *,
    ...
    # Frame 5 (steps 18-48) runs MovePTP/MoveL faster; None outside frame 5 so
    # earlier frames keep their default speeds.
    ...
    if step_number == 1:
        ...
        if bool(ctx.params["VISE_CLOSE_FLOATING_ENABLED"]):
            ctx.start_cartesian_floating(
                "float TCP Y/Z/RX while the vise clamps the inserted injectable"
            )
        ...
        ctx.gripper("open")
        prepare_arduino_for_vise_action(
            ctx, action_label="CLOSE_VISE after injectable release confirmation"
        )
        result = require_arduino(ctx).close_vise(
            target_force_kg=float(ctx.params["VISE_TARGET_FORCE_KG"])
        )
        ...
        ctx.logger.info(
            "CUT_HEIGHT complete; re-tightening vise before cap re-grip/twist"
        )
        result = arduino.close_vise(
            target_force_kg=float(ctx.params["VISE_TARGET_FORCE_KG"])
        )
        ctx.logger.info(f"Post-cut CLOSE_VISE result: {result}")
        validate_vise_closed(result, float(ctx.params["VISE_TARGET_FORCE_KG"]))
    elif step_number == 14:
        cap_world_pose = cap_grip_world_pose(ctx)
        ctx.move_l(
            "cap_grip_world_z",
            cap_world_pose,
        ...
        ctx.move_l(
            "cap_grip_tcp_z",
            cap_grip_tcp_pose(ctx, cap_world_pose),
            vel_m_s=float(ctx.params["CARTESIAN_INSERT_VEL_M_S"]),
        )
    elif step_number == 15:
        if bool(ctx.params["VISE_CLOSE_FLOATING_ENABLED"]):
            ctx.start_cartesian_floating(
                "float TCP Y/Z/RX while closing gripper on the cap"
            )
        ctx.gripper("close")
    elif step_number == 16:
        if bool(ctx.params["VISE_CLOSE_FLOATING_ENABLED"]):
            ctx.hold_current_joints(
                "freeze current pose after floating cap grip before twist"
            )
            ctx.switch_to_primitive_mode("resume motion control for cap twist")
        ctx.twist_cap()
    elif step_number == 17:
        ctx.move_current_z_offset(
            "cap_lift",
...
def run_robot_only_smoke(ctx: RecipeContext, *, uncut: bool = False) -> None:
    ...
    ctx.logger.warn(
        "Robot-only smoke skips Arduino CLOSE_VISE, post-release CLOSE_VISE confirmation, "
        "and CUT_HEIGHT; "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Loop the full recipe continuously until Ctrl-C.",
    )
