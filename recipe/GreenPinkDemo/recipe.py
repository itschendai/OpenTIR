...
PARAMS: dict[str, Any] = {
    ...
    # Frame 5 legacy teardown steps (21-48: spring/plastic/shell/glass
    # removal + dump) handle already-processed scrap, so their MovePTP
    # transits and MoveL lifts/descents run faster than the delicate
    # pickup/insert/cut demo frames. Set FRAME5_FAST_ENABLED False to make
    # those legacy teardown steps use the same speeds as the demo path.
    ...
    "VISE_CLOSE_FLOATING_ENABLED": True,
    "VISE_CLOSE_FLOATING_AXES": ["y", "z"],
    "VISE_CLOSE_FLOATING_MAX_VEL_M_S": 0.2,
    "VISE_CLOSE_FLOATING_ENABLE_ELBOW_MOTION": False,
    "VISE_CLOSE_FLOATING_ENABLE_SIX_AXIS_JNT_CTRL": False,
    ...
    "VISE_TARGET_FORCE_KG": None,
    "CUT_X_MM": 90.0,
    "CUT_Z_MM": 133.0,
    "CUT_DEG": 360.0,
    "ROT_SAFE_TOL_DEG": 0.2,
    "CAP_GRIP_Z_OFFSET_M": -0.038,
    "CAP_TWIST_DEG": 5.0,
...
PLAN_STEPS = {
    ...
    10: "close vise while floating in TCP Y-Z",
    11: "release injectable with wider gripper opening",
    12: "retreat up in world Z and wait clear of vise",
    13: "CUT_HEIGHT fake cut",
    14: "move down to Vise to re-grip injectable",
    15: "close gripper at 80 N on injectable",
    16: "open vise to release injectable",
    17: "lift injectable up in positive world Z",
    18: "move to Middle",
    19: "move to Glass",
    20: "dump with MoveC arc around tool-frame virtual pivot and return",
    49: "move to Plastic",
    50: "move to Middle",
    51: "move to Plate",
    52: "open gripper",
...
DEMO_RETURN_STEP_SEQUENCE = (14, 15, 16, 17, 18, 19, 20, 49, 50, 51, 52)
DEMO_FULL_STEP_SEQUENCE = (*tuple(range(1, 21)), 49, 50, 51, 52)
...
class RecipeContext:
    ...
    def move_ptp(
        ...
        *,
        ...
        effective_vel_scale = (
            int(self.params["MOVE_JNT_VEL_SCALE"])
            if vel_scale is None
            else int(vel_scale)
        )
        self.logger.info(
            f"MovePTP -> {label} ({_fmt_pose_mm(pose_entry)}; {ref_mode}; "
            f"jntVelScale={effective_vel_scale})"
        )
        if self.dry_run:
            return
    ...
    def move_l(self, label: str, pose_entry: dict, *, vel_m_s: float) -> None:
        self.logger.info(
            f"MoveL -> {label} ({_fmt_pose_mm(pose_entry)}; {float(vel_m_s):.3f} m/s)"
        )
        if self.dry_run:
            return
        if self.session is None:
    ...
    def move_current_z_offset(
        self, label: str, dz_m: float, *, vel_m_s: float | None = None
    ) -> None:
        effective_vel = (
            float(self.params["CARTESIAN_RETREAT_VEL_M_S"])
            if vel_m_s is None
            else float(vel_m_s)
        )
        if self.dry_run:
            self.logger.info(
                ...
                f"@ {effective_vel:.3f} m/s"
            )
            return
        if self.session is None:
            raise RecipeError("robot session is not available")
        pose = current_pose_offset_z(self.session, dz_m)
        self.move_l(label, pose, vel_m_s=effective_vel)
    ...
    def start_cartesian_floating(self, reason: str) -> None:
        axes = [str(axis).lower() for axis in self.params["VISE_CLOSE_FLOATING_AXES"]]
        max_vel_m_s = float(self.params["VISE_CLOSE_FLOATING_MAX_VEL_M_S"])
        self.logger.info(
            "Cartesian floating -> "
            f"{reason} axes={axes} maxVel={max_vel_m_s:.3f} m/s"
        )
        if self.dry_run:
            return
        if self.session is None:
            raise RecipeError("robot session is not available")
        self.session.start_cartesian_floating(
            axes,
            max_vel_m_s=max_vel_m_s,
            enable_elbow_motion=bool(
                self.params["VISE_CLOSE_FLOATING_ENABLE_ELBOW_MOTION"]
            ),
            enable_six_axis_jnt_ctrl=bool(
                self.params["VISE_CLOSE_FLOATING_ENABLE_SIX_AXIS_JNT_CTRL"]
            ),
        )
...
def validate_vise_closed(result: dict, target_force_kg: float | None) -> None:
...
def close_vise_target_force_kg(ctx: RecipeContext) -> float | None:
    value = ctx.params.get("VISE_TARGET_FORCE_KG")
    if value is None:
        return None
    return float(value)
...
def step_requires_arduino(step_number: int) -> bool:
    return step_number in {10, 13, 16, 41}
...
FRAME5_FIRST_STEP = 21


def frame5_velocity_overrides(
    ctx: RecipeContext, step_number: int
) -> tuple[int | None, float | None]:
    """Frame-5 legacy teardown steps (21-48) run faster on scrap removal.

    Returns ``(jnt_vel_scale, cartesian_vel_m_s)`` for frame-5 steps when
    ``FRAME5_FAST_ENABLED`` is set, otherwise ``(None, None)`` so callers fall
    back to the default MovePTP scale and per-move MoveL velocities used by the
    delicate pickup/insert/cut frames (1-4).
    """
    is_legacy_frame5_step = (
        removal_cycle_for_step(step_number) is not None
        or shell_glass_step_offset(step_number) is not None
    )
    if bool(ctx.params["FRAME5_FAST_ENABLED"]) and is_legacy_frame5_step:
        return (
            int(ctx.params["FRAME5_MOVE_JNT_VEL_SCALE"]),
            float(ctx.params["FRAME5_CARTESIAN_VEL_M_S"]),
        )
    return (None, None)
...
def execute_plan_step(
    ...
    *,
    ...
    # Only the legacy teardown steps use the faster frame-5 overrides; the demo
    # pickup/load/return path keeps its default speeds.
    ptp_vel_scale, frame5_cart_vel = frame5_velocity_overrides(ctx, step_number)
    insert_vel = (
        frame5_cart_vel
        if frame5_cart_vel is not None
        else float(ctx.params["CARTESIAN_INSERT_VEL_M_S"])
    )
    ...
    if removal_step is not None:
        ...
        if action == "transit":
            ...
            ctx.move_ptp(transit_label, transit_entry, vel_scale=ptp_vel_scale)
            ...
            ctx.move_current_z_offset(f"{name}_lift", lift_z_m, vel_m_s=frame5_cart_vel)
        elif action == "return_transit":
            transit_label, transit_entry = transit_pose(ctx)
            ctx.move_ptp(transit_label, transit_entry, vel_scale=ptp_vel_scale)
        elif action == "drop":
            drop_label, drop_pose, _ = resolve_drop_pose(ctx, str(cycle["drop_target"]))
            ctx.move_ptp(drop_label, drop_pose, vel_scale=ptp_vel_scale)
        elif action == "open":
            ctx.gripper("open")
        return
    ...
    if shell_offset is not None:
        if shell_offset == 0:
            ...
            ctx.move_current_z_offset(
                ...
                vel_m_s=frame5_cart_vel,
            )
        elif shell_offset == 6:
            transit_label, transit_entry = transit_pose(ctx)
            ctx.move_ptp(transit_label, transit_entry, vel_scale=ptp_vel_scale)
        elif shell_offset == 7:
            ctx.move_ptp("Glass", ctx.poses["Glass"], vel_scale=ptp_vel_scale)
        elif shell_offset == 8:
            ctx.dump_tool_z()
        elif shell_offset == 9:
            ctx.move_ptp("Plastic", ctx.poses["Plastic"], vel_scale=ptp_vel_scale)
    ...
    if step_number == 1:
        ...
        if bool(ctx.params["VISE_CLOSE_FLOATING_ENABLED"]):
            ctx.start_cartesian_floating(
                "float TCP Y/Z while the vise clamps the inserted injectable"
            )
        prepare_arduino_for_vise_action(ctx, action_label="CLOSE_VISE")
        target_force_kg = close_vise_target_force_kg(ctx)
        if target_force_kg is None:
            result = require_arduino(ctx).close_vise()
        else:
            result = require_arduino(ctx).close_vise(target_force_kg=target_force_kg)
        validate_vise_closed(result, target_force_kg)
    elif step_number == 11:
        ...
        stage_label, stage_pose = pickup_stage_pose(ctx)
        ctx.move_ptp(stage_label, stage_pose, vel_scale=ptp_vel_scale)
    elif step_number == 52:
        ctx.gripper("open")
...
def phase_return_injectable_to_plate(ctx: RecipeContext) -> None:
    ctx.logger.info("=== phase_return_injectable_to_plate ===")
    run_plan_steps(ctx, DEMO_RETURN_STEP_SEQUENCE)
...
def run_full_recipe(
    ...
    *,
    ...
    if uncut:
        ctx.logger.info(
            "=== uncut shortcut enabled: after step 12 skip CUT_HEIGHT and return the injectable to Plate ==="
        )
    else:
        phase_cut(ctx, yes=yes, skip_cut=skip_cut)
    phase_return_injectable_to_plate(ctx)
...
def run_robot_only_smoke(ctx: RecipeContext, *, uncut: bool = False) -> None:
    ...
    ctx.logger.warn(
        "Robot-only smoke skips Arduino CLOSE_VISE, CUT_HEIGHT, and OPEN_VISE; "
    ...
    step_numbers = UNCUT_STEP_SEQUENCE if uncut else DEMO_FULL_STEP_SEQUENCE
    if uncut:
        ctx.logger.info(
            "Robot-only smoke uncut path: after step 12 skip CUT_HEIGHT and return the injectable to Plate"
        )
    for step_number in step_numbers:
...
def restart_arduino_session_for_demo_cycle(ctx: RecipeContext) -> None:
    if ctx.arduino is None:
        return
    ctx.logger.info("Demo loop: restarting Arduino session for the next cycle")
    ctx.arduino.close()
    ctx.arduino.connect()
    ctx.arduino_status_query_supported = None
    ...
    *,
    ...
    if uncut:
        logger.info(
            ...
            "skip_cut -> return_injectable_to_plate -> shutdown"
        )
    else:
        logger.info(
            "phase_order=startup_transfer -> calibrate_tags -> "
            "pick_from_camera_align_grasp -> load_vise -> cut -> "
            "return_injectable_to_plate -> shutdown"
        )
    ...
    for key in (
        ...
        "VISE_TARGET_FORCE_KG",
        "DUMP_TOOL_Z_DEG",
        "VISE_OPEN_TARGET_FORCE_KG",
        "MOVE_JNT_VEL_SCALE",
        "MOVE_USE_REF_JOINTS",
        "FRAME5_FAST_ENABLED",
        "FRAME5_MOVE_JNT_VEL_SCALE",
        "FRAME5_CARTESIAN_VEL_M_S",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Loop the GreenPinkDemo cycle continuously until Ctrl-C.",
    )
    parser.add_argument("--skip-cut", action="store_true", help="Run all phases except CUT_HEIGHT")
    parser.add_argument(
        "--uncut",
        action="store_true",
        help="After step 12, skip CUT_HEIGHT and return the injectable to Plate without firing the fake cut.",
    )
    parser.add_argument(
        "--slow",
        action="store_true",
    ...
    parser.add_argument("--vise-target-force-kg", type=float, default=None)
...
def resolve_params(args: argparse.Namespace) -> dict:
    ...
    overrides = {
        ...
        "VISE_TARGET_FORCE_KG": args.vise_target_force_kg,
...
def run_requested_recipe_mode(
    ...
    *,
    ...
    if selected_steps is not None:
        ...
        return
    if args.robot_only_smoke:
        run_robot_only_smoke(ctx, uncut=args.uncut)
        return
    if args.demo:
        cycle = 1
        while True:
            ctx.logger.info(f"=== demo cycle {cycle} start ===")
            run_full_recipe(
                ctx,
                yes=args.yes,
            ...
            ctx.logger.info(f"=== demo cycle {cycle} complete ===")
            restart_arduino_session_for_demo_cycle(ctx)
            cycle += 1
        return
    run_full_recipe(
        ctx,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.demo and args.step is not None:
            raise RecipeError("--demo cannot be combined with --step")
        if args.demo and args.tag_calibration_only:
            raise RecipeError("--demo cannot be combined with --tag-calibration-only")
        if args.demo and args.robot_only_smoke:
            raise RecipeError("--demo cannot be combined with --robot-only-smoke")
        if args.demo and args.dry_run:
            raise RecipeError("--demo cannot be combined with --dry-run")
        if not args.dry_run:
        ...
        if args.dry_run:
            ...
            run_requested_recipe_mode(
                ctx,
                argparse.Namespace(**{**vars(args), "yes": True, "demo": False}),
        ...
        try:
            ...
            with RobotSession(params["ROBOT_SN"], logger=logger) as session:
                ...
                run_requested_recipe_mode(ctx, args, selected_steps=selected_steps)
            return 0
        except Exception:
