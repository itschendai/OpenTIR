...
- The only rotation left in the demo path is the machine-side fake cut; any
  robot `MoveC` dump motion only applies to the legacy teardown steps.
- **Legacy frame 5 fast mode** (steps 21-48, `FRAME5_FAST_ENABLED = True`):
  teardown-only scrap removal runs faster than the delicate frames 1-4. There,
  MovePTP uses
  ...
  those legacy teardown steps fall back to the frames 1-4 speeds.
...
10. Machine closes vise while the robot floats in the TCP Y-Z plane (`FloatingCartesian` in TCP START frame on axes `y,z`). — Arduino/machine action + robot floating max 0.2 m/s (`VISE_CLOSE_FLOATING_MAX_VEL_M_S`, controller minimum)
11. Robot exits the compliant insertion hold / TCP Y-Z floating state, switches to joint hold / non-force control at the current pose, then opens gripper to 5 cm width to release the injectable. — gripper 0.05 m/s (`GRIPPER_VELOCITY_M_S`)
12. While still in joint hold, robot opens gripper fully, waits for gripper to finish opening, then switches back to motion primitive control and moves straight up in world Z by 15 cm to avoid hitting the injectable and wait. — gripper 0.05 m/s; retreat MoveL 0.06 m/s (`CARTESIAN_RETREAT_VEL_M_S`)

//frame 4
13. Machine starts cut height primitive (`cut_height z_mm=133 x_mm=90 deg=360`) to fake a Cut
14. Robot moves back down to a bit below the Vise position (Vise world Z -3.8 cm) to re-grip the injectable. — MoveL 0.06 m/s (`CARTESIAN_INSERT_VEL_M_S`)
15. Robot closes gripper to 80 N on the injectable. — gripper 0.05 m/s (`GRIPPER_VELOCITY_M_S`)
16. Machine opens vise to release the injectable back to the robot. — Arduino/machine action (no robot speed)
17. Robot slowly raises up in **positive world Z** by 20 cm with the injectable. — MoveL 0.06 m/s (`CARTESIAN_RETREAT_VEL_M_S`)
18. Robot moves to Middle point first to clear the vise area before crossing toward Glass. — MovePTP @ 30% joint-vel scale (no fixed m/s)
19. Robot moves to Glass point. — MovePTP @ 30% joint-vel scale (no fixed m/s)
20. The robot performs Dump with one continuous MoveC arc around an upper-left virtual point in the tool frame (Rz 176 and then turn back). — MoveC 0.03 m/s TCP linear (`DUMP_MOVEC_VEL_M_S`, unchanged); the pivot is ~28 mm from the TCP, so this works out to ~61 deg/s angular (derived, not commanded directly)
21. Robot moves to Plastic point. — MovePTP @ 30% joint-vel scale (no fixed m/s)
22. Robot moves to Middle point. — MovePTP @ 30% joint-vel scale (no fixed m/s)
23. Robot moves to Plate point. — MovePTP @ 30% joint-vel scale (no fixed m/s)
24. Robot opens gripper to drop the injectable back on Plate. — gripper 0.05 m/s (`GRIPPER_VELOCITY_M_S`)
...
8. Let the Arduino close the vise while the robot floats in TCP Y-Z to accommodate clamp-side alignment.
9. Exit the compliant insertion hold / TCP Y-Z floating state, park the arm in joint hold for the gripper-open actions, then switch back to motion primitive control and retreat to a safe waiting pose.
10. Let the Arduino run `CUT_HEIGHT` with `z=133`, `x=90`, `deg=360` to fake
   the cut while the robot stays clear.
11. Return to relocalized `Vise`, re-grip the intact injectable, open the vise,
   lift it out in positive world Z, then go `Middle -> Glass -> Dump ->
   Plastic -> Middle -> Plate` and place it back onto `Plate`.
...
| `plastic` | `plastic.json` | Legacy teardown drop pose; not used in the demo return path. |
| `spring` | `spring.json` | Legacy teardown handling pose; not used in the demo return path. |
| `glass` | `glass.json` | Legacy teardown handling pose; not used in the demo return path. |
...
| 11 | Hold part in gripper while floating in TCP `y,z` | `CLOSE_VISE` | Robot keeps custody until vise confirms closed, with clamp-side compliance in the TCP Y-Z plane. |
| 12 | Switch to joint hold, then open gripper to 5 cm width | None | Only after `CLOSE_VISE DONE`; exits both `InsertComp` compliance and TCP Y-Z floating before release. |
...
| 14 | Robot holds still | `CUT_HEIGHT z=133 x=90 deg=360` | Robot must be clear; this is a demo fake cut. |
| 15 | Move down in world Z to `Vise` | None | Re-enter only after the machine is safe. |
| 16 | Close gripper at 80 N on the injectable | None | Re-grip the intact part before opening the vise. |
| 17 | Hold part in gripper | `OPEN_VISE` | Robot takes custody again before lifting out. |
| 18 | Lift by positive world Z +20 cm | None | Clear the vise with the injectable still in hand. |
| 19 | Move to `Middle` | None | First clear the vise area before heading toward `Glass`. |
| 20 | Move to `Glass` | None | Match the `--uncut` teardown transit pattern before returning. |
| 21 | Run dump arc at `Glass` | None | Same MoveC dump motion used by the uncut shell/glass path. |
| 22 | Move to `Plastic` | None | Transit only; keep gripper closed here. |
| 23 | Move to `Middle` | None | Safe transit away from the disposal side. |
| 24 | Move to `Plate` | None | Return location for the intact injectable. |
| 25 | Open gripper | None | Release the injectable back onto `Plate`. |
...
6. `phase_cut()`
   - verify robot clear
   - `CUT_HEIGHT x=90 z=133 deg=360`
   - verify blade off, X/Z returned home, and rotary is within tolerance

7. `phase_return_injectable_to_plate()`
   - move back down to captured `Vise`
   - close gripper and wait for firm grip settle
   - `OPEN_VISE`
   - lift by positive world Z +20 cm with the injectable
   - move to `Middle` first for collision-safe clearance
   - move to `Glass`
   - perform `Dump`
   - move to `Plastic` without opening the gripper
   - move `Middle`
   - move to `Plate`
   - open gripper
...
- skips step 16 `OPEN_VISE`
- still runs the robot-only return path: Vise re-entry, injectable close,
  positive world-Z lift, `Glass`, `Dump`, `Plastic` transit, `Middle`,
  `Plate`, and gripper open
...
```bash
python3 project/recipe/GreenPinkDemo/recipe.py --dry-run --step 14
python3 project/recipe/GreenPinkDemo/recipe.py --step 14
python3 project/recipe/GreenPinkDemo/recipe.py --dry-run --step 17
```
...
- Is the saved `Plate` pose safe enough for re-placing the injectable without a second camera align?
- Does the current `OPEN_VISE` target force fully release the injectable without the part shifting?
- What are `spring`, `plastic`, and `glass` intended to do in this recipe now that they are legacy teardown-only?
...
5. Add `CLOSE_VISE`.
6. Add `CUT_HEIGHT`.
7. Add fake `CUT_HEIGHT`, re-grip in the vise, `OPEN_VISE`, world-Z lift, and return to `Plate`.
