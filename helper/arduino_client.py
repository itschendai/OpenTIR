...
Copied forward from ``project/injectable_pipeline/arduino_client.py`` so recipes
that use the vise / cutter can share one tested serial client. Implements the
line-based wire format in ``XZ Stage Code v2/PRIMITIVES.md``: one mutating command
at a time; queries return immediately; responses are line-based ASCII at 115200
baud.
"""
