"""Flexiv RDK convenience helpers shared by project recipes.
...
Copied forward from ``project/injectable_pipeline/flexiv_helpers.py`` so new recipes
can share the proven helpers instead of rewriting them in each recipe script.
"""
...
HOLD_CURRENT_JOINTS_MAX_VEL = 1.0
HOLD_CURRENT_JOINTS_MAX_ACC = 2.0
...
FLOATING_CARTESIAN_INERTIA_SCALE = [1.0] * 6
FLOATING_FRICTION_COMP_SCALE = [100.0] * 7
FLOATING_CARTESIAN_MIN_MAX_VEL_M_S = 0.2
...
def _sized_joint_vector(values: Iterable[float], dof: int) -> list[float]:
    items = [float(v) for v in values]
    if len(items) >= dof:
        return items[:dof]
    return items + [items[-1] if items else 0.0] * (dof - len(items))


def _dof_from_state(robot, state) -> int:
    q = _vector(getattr(state, "q", []))
    if q:
        return len(q)
    return int(robot.info().DoF)


def _floating_response_torque(dof: int) -> list[float]:
    return [
        value * FLOATING_LOAD_COMPENSATION_SCALE
        for value in _sized_joint_vector(FLOATING_RESPONSE_TORQUE, dof)
    ]


def floating_coord_tcp_start(flexivrdk_module=None):
    flexivrdk = flexivrdk_module or _load_flexivrdk()
...
def cartesian_axis_mask_for_selection(axis_names: Iterable[str] | None) -> list[int]:
    if not axis_names:
        return [1] * 6
    mask = [0] * 6
    for axis_name in axis_names:
        normalized = str(axis_name).lower()
        if normalized not in FLOATING_CARTESIAN_AXIS_NAMES:
            raise ValueError(
                f"unsupported Cartesian floating axis {axis_name!r}; "
                f"choose from {FLOATING_CARTESIAN_AXIS_NAMES}"
            )
        mask[FLOATING_CARTESIAN_AXIS_NAMES.index(normalized)] = 1
    return mask
...
def _state_value_matches(value, expected) -> bool:
    if isinstance(value, (list, tuple)):
        return all(_state_value_matches(item, expected) for item in value)
    if isinstance(expected, bool):
        return bool(value) is expected
    return value == expected


def _primitive_state_matches(robot, state_key: str, expected) -> bool:
    states = robot.primitive_states()
    if isinstance(states, dict):
        if state_key in states:
            return _state_value_matches(states[state_key], expected)
        if not states:
            return False
        for state in states.values():
            value = _primitive_state_value(state, state_key)
            if value is None or not _state_value_matches(value, expected):
                return False
        return True
    return False
...
def wait_for_primitive_state(
    ...
    """Poll ``robot.primitive_states()`` until ``state_key`` equals ``expected``."""
    deadline = None if timeout_s is None else time.monotonic() + max(0.0, timeout_s)
    while not _primitive_state_matches(robot, state_key, expected):
        if deadline is not None and time.monotonic() >= deadline:
            raise TimeoutError(
                f"Primitive state {state_key!r} did not become {expected!r} "
...
def rotate_tcp_about_tool_axis(
    tcp_pose,
    *,
    roll_deg: float = 0.0,
    pitch_deg: float = 0.0,
    ...
    """Return TCP pose after an in-place local tool-frame RPY rotation."""
    pose = [float(v) for v in tcp_pose]
    if len(pose) != 7:
        raise ValueError(f"tcp_pose must have 7 values, got {len(pose)}")
    delta_quat = rpy_deg_to_quat(roll_deg, pitch_deg, yaw_deg)
    target_quat = _quat_multiply(pose[3:], delta_quat)
    return pose[:3] + list(target_quat)
...
def move_ptp_joint(
    ...
    *,
    ...
    use_ref_joints: bool = True,
) -> None:
    """MovePTP to a captured task-space pose.

    Reads ``tcp_pose_world`` from ``pose_entry`` as the target. If
    ``use_ref_joints=True``, also passes ``q_rad`` as ``refJntPos`` so IK picks
    a configuration near the operator's demonstrated arm shape.
    ...
    if joint_locked:
        use_ref_joints = True
    joints_deg = joints_to_jpos_deg(pose_entry["q_rad"]) if use_ref_joints else None
    coord_args = tcp_pose_to_coord_args(
        tcp,
        ref_joints_deg=joints_deg,
        include_default_refs=use_ref_joints,
    )
    primitive_params = {
        "target": flexivrdk.Coord(*coord_args),
        "jntVelScale": int(vel_scale),
    ...
    if use_ref_joints:
        primitive_params["enableFixRefJntPos"] = bool(joint_locked)
        primitive_params["refJntPos"] = flexivrdk.JPos(joints_deg)
    session.execute_primitive("MovePTP", primitive_params)
    session.wait_for_primitive("reachedTarget")
...
def tcp_pose_to_coord_args(
    ...
    include_default_refs: bool = True,
) -> tuple:
    """Build positional arguments for ``flexivrdk.Coord``.
    ...
    Set ``include_default_refs=False`` to return only the official task-space
    ``Coord(position, orientation, ref_frame)`` arguments when no reference
    joints or external axes are intended.
    """
    pose = [float(v) for v in tcp_pose]
    if len(pose) != 7:
        raise ValueError(f"tcp_pose must have 7 values, got {len(pose)}")
    ...
    if (
        not include_default_refs
        and ref_joints_deg is None
        and ref_external is None
    ):
        return (position, orientation, list(ref_frame))
...
class RobotSession:
    ...
    def hold_current_joints(self) -> None:
        """Stop the active primitive, switch to joint hold, and latch current q.

        This is useful after compliant primitives such as ``InsertComp`` when we
        want the arm to stay rigidly parked during a gripper-only action.
        """
        try:
            self.robot.Stop()
        except Exception as err:  # noqa: BLE001 - best-effort mode transition
            self._warn(f"robot.Stop() before joint hold raised: {err}")
        self.switch_mode("NRT_JOINT_IMPEDANCE")

        info = self.robot.info()
        if not hasattr(info, "K_q_nom"):
            raise RuntimeError(
                "robot.info().K_q_nom is unavailable; cannot restore nominal stiffness"
            )
        self.robot.SetJointImpedance(_vector(info.K_q_nom))

        _, hold_state = self.selected_arm_state()
        q = _vector(getattr(hold_state, "q", []))
        if not q:
            raise RuntimeError("current joint state is unavailable; cannot hold joints")
        dof = len(q)
        self.robot.SendJointPosition(
            q,
            [0.0] * dof,
            [HOLD_CURRENT_JOINTS_MAX_VEL] * dof,
            [HOLD_CURRENT_JOINTS_MAX_ACC] * dof,
        )
        self._info("Robot joint hold is active at the current configuration")

    def start_cartesian_floating(
        self,
        axis_names: Iterable[str] | None = None,
        *,
        max_vel_m_s: float = 0.05,
        enable_elbow_motion: bool = False,
        enable_six_axis_jnt_ctrl: bool = False,
    ) -> None:
        """Start Flexiv ``FloatingCartesian`` in the TCP START frame.

        ``axis_names`` uses TCP-frame axis labels ``x/y/z/rx/ry/rz``. Any axis
        omitted from the selection stays constrained by the primitive.
        """
        _, state = self.selected_arm_state()
        dof = _dof_from_state(self.robot, state)
        floating_axis = cartesian_axis_mask_for_selection(axis_names)
        requested_max_vel_m_s = float(max_vel_m_s)
        effective_max_vel_m_s = max(
            FLOATING_CARTESIAN_MIN_MAX_VEL_M_S, requested_max_vel_m_s
        )
        if effective_max_vel_m_s != requested_max_vel_m_s:
            self._warn(
                "FloatingCartesian maxVel "
                f"{requested_max_vel_m_s:.3f} m/s is below the controller minimum "
                f"{FLOATING_CARTESIAN_MIN_MAX_VEL_M_S:.3f} m/s; "
                f"using {effective_max_vel_m_s:.3f} m/s"
            )
        try:
            self.robot.Stop()
        except Exception as err:  # noqa: BLE001 - best-effort mode transition
            self._warn(f"robot.Stop() before Cartesian floating raised: {err}")
        self.switch_mode("NRT_PRIMITIVE_EXECUTION")
        self.execute_primitive(
            "FloatingCartesian",
            {
                "floatingAxis": floating_axis,
                "enableElbowMotion": bool(enable_elbow_motion),
                "floatingCoord": floating_coord_tcp_start(self._flexivrdk),
                "dampingLevel": list(FLOATING_CARTESIAN_DAMPING_LEVEL),
                "enableSixAxisJntCtrl": bool(enable_six_axis_jnt_ctrl),
                "diEnableFloating": "NONE",
                "responseTorque": _floating_response_torque(dof),
                "inertiaScale": list(FLOATING_CARTESIAN_INERTIA_SCALE),
                "maxVel": effective_max_vel_m_s,
                "frictionCompScale": _sized_joint_vector(
                    FLOATING_FRICTION_COMP_SCALE, dof
                ),
            },
        )
        axes_label = (
            ", ".join(str(axis).lower() for axis in axis_names)
            if axis_names
            else "all Cartesian axes"
        )
        self._info(
            "Robot Cartesian floating is active in TCP START frame on "
            f"{axes_label} (maxVel={effective_max_vel_m_s:.3f} m/s)"
        )
